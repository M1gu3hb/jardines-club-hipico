...
export default function AdminServicioItems() {
  ...
  const save = async () => {
    ...
    if (editing === "new") await base44.entities.ServicioItem.create(form);
    else await base44.entities.ServicioItem.update(editing, form);
    cancel(); load();
  };

  const remove = async (id) => { await base44.entities.ServicioItem.delete(id); load(); };
  const toggle = async (item) => { await base44.entities.ServicioItem.update(item.id, { activo: !item.activo }); load(); };
  ...
  const move = async (i, dir) => {
    ...
    await Promise.all(next.map((it, idx) => base44.entities.ServicioItem.update(it.id, { orden: idx + 1 })));
    load();
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-white text-xl font-light">Servicios</h2>
...
function ItemForm({ form, setForm, onSave, onCancel }) {
  ...
  return (
    <div className="border border-[#C9A84C]/20 bg-[#C9A84C]/3 p-5 mb-3 space-y-3">
      ...
      <button type="button" onClick={() => set("portalSugerible", !form.portalSugerible)}
        className="flex items-center gap-2.5 text-left pt-1">
        <span className={`w-5 h-5 rounded-sm border flex items-center justify-center transition-all flex-shrink-0 ${form.portalSugerible ? "border-[#C9A84C] bg-[#C9A84C]" : "border-white/20"}`}>
          {form.portalSugerible && <Check size={10} className="text-black" />}
        </span>
        <span>
          <span className="text-white/60 text-xs">Sugerir en el portal del cliente</span>
          <span className="block text-white/25 text-[11px]">Actívalo solo si es un extra contratable (no un servicio incluido).</span>
        </span>
      </button>
