/**
 * StaggeredMenu — menú de secciones del sitio (adaptado de React Bits, dep: gsap).
 * Adaptaciones para Jardines Club Hípico:
 *  - Tema oscuro (#0a0a0a) + acento dorado #C9A84C, tipografía Inter, letras más chicas.
...
export default function StaggeredMenu({
  ...
  const animateText = useCallback((opening) => {
    ...
    for (let i = 0; i < cycles; i++) { last = last === "Menú" ? "Cerrar" : "Menú"; seq.push(last); }
  ...
  React.useEffect(() => {
    ...
    const handleClickOutside = (event) => {
      if (panelRef.current && !panelRef.current.contains(event.target) &&
          toggleBtnRef.current && !toggleBtnRef.current.contains(event.target)) closeMenu();
    };
    document.addEventListener("mousedown", handleClickOutside);
    return () => document.removeEventListener("mousedown", handleClickOutside);
  }, [closeOnClickAway, open, closeMenu]);

  const handleItem = (e, it) => {
    e.preventDefault();
    onItemClick?.(it);
    closeMenu();
  };
  ...
  return (
    ...
    >
      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {(() => {
          const raw = colors && colors.length ? colors.slice(0, 4) : ["#141414", "#20180a"];
          let arr = [...raw];
          if (arr.length >= 3) { const mid = Math.floor(arr.length / 2); arr.splice(mid, 1); }
          return arr.map((c, i) => <div key={i} className="sm-prelayer" style={{ background: c }} />);
        })()}
      </div>
      {/* La clase `con-scroll` NO la pone React: la pone un guion de `index.html` sobre el
          elemento `<html>`, y el CSS de aquí reacciona a ella. Ver la nota en ese archivo. */}
