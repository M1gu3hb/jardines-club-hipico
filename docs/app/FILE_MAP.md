...
| Archivo | Qué hace |
|---|---|
| `api/solicitud.js` | **La única ruta.** Recibe `{ solicitudId }`, relee la fila con `service_role`, y manda **dos correos independientes** con los datos canónicos de la base: el aviso al dueño y, **solo si la fila trae correo**, el acuse a quien llenó el formulario (folio, resumen de lo pedido y plazo de respuesta). Rate limit por IP (10/hora), ventana de 15 minutos, **una clave de idempotencia por correo** (`solicitud-correo` y `solicitud-acuse`), todo escapado con `escHtml`, y auditoría de cada resultado (`solicitud_correo` / `solicitud_acuse`). El 500 lo decide solo el aviso al dueño. Segundo botón de WhatsApp solo si el teléfono se pudo derivar. Exporta además `acuseParaCliente`, pura, para que la suite pueda ejercitar el correo sin mandarlo |
